...
- **Update, M-11 debt audit, 2026-09-22:** Still missing — `docs/PROJECT_INSTRUCTIONS.md` doesn't exist, and `docs/PROJECT_MAP.md` still points at it twice (its own reading-order table and its doc-tree diagram). The original trigger ("before M-2 begins") was missed without ever being flagged: M-2 through M-11 have all shipped with this dead reference still in place. Left as `open` rather than inventing a new deadline unilaterally — that's a scheduling call, not a doc-accuracy one — but the "before M-2" framing is no longer meaningful and should be replaced with a real trigger the next time this file is reviewed with Christian.
- **Owner:** Christian
- **Status:** open
...
- **Update, M-11 debt audit, 2026-09-22:** M-10 (Replay Engine) and M-9 (Simulator UI, built on top of it) have both since shipped — the "before M-10" trigger has arrived, not just approached. `import_yfinance.py` still pulls `interval="1d"` only; nothing intraday has been seeded. `/replay/page.tsx` names TD-05 explicitly as the reason it hardcodes the timeframe to `"1d"` instead of offering the same selector `/chart` has, so the debt is actively load-bearing on a shipped route today, not a latent gap. Risk is still correctness-neutral (empty state, not a wrong number) so this isn't urgent, but the trigger wording should read as current, not future, the next time this is reviewed.
- **Owner:** Christian
- **Status:** open
